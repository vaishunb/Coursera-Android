					...
					public void onReceive(Context context, Intent intent) {
						...
						if (getResultCode() != MainActivity.IS_ALIVE) {
                            ...
                            PendingIntent pendingIntent = PendingIntent.getBroadcast(context, 0,
                                    restartMainActivtyIntent, 0);
                            restartMainActivtyIntent.setFlags(PendingIntent.FLAG_UPDATE_CURRENT);
							...
							// setAutoCancel(true).
                            Notification resultNotification = new Notification.Builder(context)
                                    .setSmallIcon(android.R.drawable.stat_sys_warning)
                                    .setAutoCancel(true).setContent(mContentView).build();
							
							// TODO: Send the notification
                            NotificationManager notificationManager = (NotificationManager) context.getSystemService(Context.NOTIFICATION_SERVICE);
                            notificationManager.notify(MY_NOTIFICATION_ID, resultNotification);
